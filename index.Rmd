...
```{r read_data_file}
read_data_file <- function(location){
  if (location = "aws"){
    # code to handle reading from AWS
  } else{
    here::here("data", location, "page_views.csv") %>% 
      readr::read_csv()
  }
}
```
...
---
# Remember page_name = regex_stuff_to_handle_special_chars(page_name)?
- Yes...why?
...
read_page_views <- function(location = "local", process_name = FALSE){
  data_file <- dplyr::case_when(
    ...
    mutate(time = lubridate::with_tz(time, tz = "America/Chicago"))
